...
33 пункта, как только хранилище полно почти-двойников. Этот бенчмарк
...
**Ранжирование по накопленной силе возвращает почти всё.** Замена веса
узла — который определяется возрастом, потому что затухание встроено в
него — на силу, которой часы не касаются:

| важность из | R@1 при 200 | R@1 при 800 |
|---|---:|---:|
| веса узла | 50.0% | 50.0% |
| **накопленной силы** | **83.3%** | **83.3%** |

Плюс 33 пункта. На LongMemEval та же правка не меняет ничего (76.0% в
обоих случаях), потому что различать там нечего — двойников нет. Этот
контраст и есть ниша продукта, выраженная замером: один пользователь,
одна тема, к которой возвращаются месяцами, — а не склад разнородных
документов.
