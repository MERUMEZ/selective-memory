...
`tools/compare_retention.py`, 3 сида, 14 суток молчания, с контролем на
объём сообщений:

| Хранилище | Узлов | Важное | Обычное |
|---|---:|---:|---:|
| Случайный отбор | 52 | 56% | 67% |
| Скользящее окно | 49 | 100% | 94% |
| **Организм** | **34** | **100%** | **100%** |
...
**Вровень в среднем, впереди на редком.** `tools/compare_memory.py`,
равный бюджет в символах:

| Хранилище | Узлов | Всё | **Редкое** | Частое |
|---|---:|---:|---:|---:|
| Случайный отбор | 64 | 96% | **60%** | 100% |
| Скользящее окно | 63 | 100% | 100% | 100% |
| Организм | 66 | 98% | **80%** | 100% |

Среднее прячет суть. На том, что упомянуто ОДИН раз, избирательная запись
находит 80% там, где случайная выборка того же объёма находит 60%, — эти
двадцать пунктов и есть то, ради чего гейт существует. На часто
повторяющемся все дают 100%, оттого общие числа и выглядят одинаковыми.
...
Скользящее окно выигрывает именно этот стенд, и это стоит сказать прямо:
прогон короткий, и свежесть покрывает почти всё. На горизонте месяцев
это не так.
