...
Замер на LongMemEval, 500 вопросов, на умолчаниях: **R@1 70.8%, R@5
84.6%** при 24.7% записанных реплик. Тот же движок без отбора на входе и без
...
восемь.** То есть navec не добавляет к поиску ничего.

Про взвешивание слов по редкости здесь раньше стояло, что оно «не
изменило ни одного попадания». Это верно ДЛЯ ЭТОГО СТЕНДА и только: на
шестнадцати фактах почти каждое слово встречается один раз, и редкость не
отличает никого ни от кого. На стогах LongMemEval в сотни реплик та же
правка подняла предпочтения с 20% до 60% R@1 — см. раздел 2.20 аудита.
