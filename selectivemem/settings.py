...
class MemorySettings:
    ...
    # предела.
    #
    # УМОЛЧАНИЕ ОСТАВЛЕНО НУЛЁМ, хотя 0.25 выглядело починкой. Замер:
    #
    #     стенд порядка (похвала)     доля важного в топ-5  83.3% -> 100.0%
    #     стенд двойников (возвраты)  R@1 при 12 повторах   83.3% ->  61.1%
    #
    # Плечо усиливает всё, что перевалило за единицу, — в том числе
    # двойников, пролезших в выдачу и получивших прибавку за извлечение.
    # Раньше потолок им мешал; теперь нет, и ошибка первого ранга
    # закрепляется. Стенд двойников — это заявленная ниша продукта, и
    # менять её на долю в топ-5 нельзя.
    #
    # Ручка оставлена и названа: прежнее поведение было безымянным
    # min(1.0, own) в глубине формулы, и то, что две трети шкалы
    # strength_max не влияют на выдачу, обнаружилось только случайно.
    strength_headroom: float = 0.0
