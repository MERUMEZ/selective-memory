...
class MemorySettings:
    ...
    #
    # МЕХАНИЗМ РАБОТАЕТ. Замер после 2360 показов: кот/кошка +0.935,
    # кот/бетон +0.080. Разделение +0.855 против перевёрнутого −0.160.
    #
    # НО ЗАДАЧУ БИБЛИОТЕКИ НЕ РЕШАЕТ, и потому ВЫКЛЮЧЕНО.
    # tools/probe_semantic.py, шестнадцать пар «факт → вопрос о нём»:
    #     встроенная модель        9/16
    #     выращенное, без опыта    8/16   (= совпадение слов)
    #     выращенное, 2303 показа  6/16
    #     выращенное, 6823 показа  6/16
    # Опыт не помогает, а мешает: стенду нужно ОБЩЕЕ ЗНАНИЕ ЯЗЫКА (что
    # Python — язык программирования), а из тысяч фраз одного человека оно
    # не выводится, сколько их ни показывай.
    #
    # И второе, устройственное: включение ОТКЛЮЧАЕТ готовую модель — иначе
    # векторы шли бы из двух пространств и косинус между ними ничего не
    # значил бы. Значит незрелый организм остаётся без смысла вовсе.
    # Подробности и обе ошибки по дороге — в шапке perception.py.
