...
class MemoryGraph(
    ...
    def __init__(
        ...
        #
        # КОГДА ОНО ВКЛЮЧАЕТСЯ. Только если различать больше нечем: своего
        # кодировщика не дали И встроенной модели в системе нет. Замер на
        ...
        #
        # Отсюда и правило. Там, где модель есть, выращенное её ЗАМЕНИЛО БЫ
        # и ухудшило поиск — смешивать два пространства нельзя, косинус
        # между ними ничего не значит. А там, где модели нет, выбор стоит
        # не между 7 и 9, а между 7 и 5.
        self.perception: Optional[GrownEncoder] = None
        if encoder is None and self._should_grow_perception():
    ...
    def _should_grow_perception(self) -> bool:
        """
        Отращивать ли смыслы самому.

        `grow_perception=True` — прямое требование хозяина, спорить не о
        чем. Иначе решаем сами и включаем ТОЛЬКО как запасной выход: когда
        семантика вообще нужна, но взять её неоткуда.

        Проверка доступности модели грузит её, если она есть. Это не
        лишняя работа: тот же вызов случился бы при первом же поиске, мы
        лишь переносим его на открытие базы. А когда модели нет, попытка
        обрывается на неудачном импорте и стоит копейки.
        """
        if self.settings.grow_perception:
            return True
        if not self.settings.embeddings_enabled:
            # Семантику выключили осознанно — не подсовываем свою взамен.
            return False
