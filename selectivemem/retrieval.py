...
class RetrievalMixin:
    ...
    def search(
        ...
        # РЕШЕНИЕ ПРИНИМАЕТСЯ ЗДЕСЬ, ПО ПРЯМОЙ УМЕСТНОСТИ, и запоминается.
        #
        # Позже достраивание поднимет счёт узлам, к которым сходятся связи,
        # — в том числе замене. Если судить по счёту ПОСЛЕ него, замена
        # окажется выше предшественника всегда, и тот исчезнет даже там,
        # где отвечает лучше. Живой случай: на запрос «тосты» пропадала
        # единственная запись со словом «тосты».
        hidden = self._superseded_to_hide(scored)
        scored = [m for m in scored if m.id not in hidden]
        ...
        top_matches = [m for m in top_matches if m.id not in hidden]
        return top_matches

    def _superseded_to_hide(self, scored):
        """
        Какие из найденных узлов НЕ отдавать: их заменили, и замена
        отвечает на этот запрос лучше.

        ДВА УСЛОВИЯ, И ОБА ОГРАНИЧИТЕЛЬНЫЕ. Если замены в выдаче нет —
        устаревшее остаётся: поправка могла быть ошибочной, и лучше
        отдать старую версию, чем не отдать ничего. Если замена стоит
        НИЖЕ — тоже остаётся: значит вопрос как раз про то, чего в замене
        нет.

        Прежнее ослабление узла такого выбора не оставляло вовсе: он
        слабел глобально и пропадал отовсюду.
        """
        if not scored:
            return set()
        ...
        if not stale_to_fresh:
            return set()
        # СКРЫВАЕМ, ТОЛЬКО ЕСЛИ ЗАМЕНА ПОДХОДИТ К ЗАПРОСУ ЛУЧШЕ.
        #
        # Первая версия прятала устаревшее всегда, когда замена была в
        # выдаче. Живой случай показал цену: на запрос «тосты» память
        # скрыла единственную запись со словом «тосты», подставив вместо
        # неё запись про омлет — потому что та когда-то была признана
        # поправкой (ошибочно).
        #
        # Замена вытесняет предшественника ровно там, где отвечает лучше
        # него. Где хуже — предшественник остаётся: поправка могла быть
        # ошибочной, а вопрос мог касаться как раз того, что изменилось.
        rank = {m.id: m.similarity for m in scored}
        hidden = set()
        for match in scored:
            fresh = stale_to_fresh.get(match.id)
            # ПРЯЧЕМ, ПОКА СТАРОЕ НЕ ВЫИГРЫВАЕТ С ЯВНЫМ ОТРЫВОМ.
            #
            # Строгое «замена должна быть не ниже» ломало верные поправки:
            # «мою собаку зовут Рекс» и «...Бобик» стоят почти вровень, и
            # устаревшее выходило первым. А без запаса вовсе — ломались
            # ошибочные: на запрос «тосты» пропадала единственная запись
            # со словом «тосты», уступая записи про омлет.
            #
            # Разрыв и различает эти случаи: у настоящей поправки записи
            # почти неразличимы (0.927 против такого же), у ошибочной —
            # 0.863 против 0.476.
            margin = self.settings.supersede_relevance_margin
