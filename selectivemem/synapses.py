...
class SynapsesMixin:
    ...
    def _evict_over_capacity(self) -> int:
        ...
        # ЁМКОСТЬ ОГРАНИЧИВАЕТ ГИППОКАМП, А НЕ КОРУ.
        #
        # Вытеснение по ёмкости — про наплыв свежих событий, и в мозге оно
        # перезаписывает гиппокампальные следы. Корковое знание тем и
        # ценно, что переживает их: оно выведено из многих случаев и не
        # принадлежит ни одному.
        #
        # Замер поймал обратное: выведенная тема «кофе» с шестью встречами
        # вылетала под напором двадцати новых эпизодов, и память забывала
        # то, что успела обобщить.
        rows = [
            row for row in self.gate.node.all()
            if not row["is_meta"] and row["node_type"] == "episodic"
        ]
