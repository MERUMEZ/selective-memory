...
SYSTEM = (
    ...
    "\n\nЕСЛИ реплика человека обнаруживает УСТОЙЧИВОЕ ПРЕДПОЧТЕНИЕ — "
    "вкус, привычку, ограничение, — добавь последней строкой ответа:\n"
    "ПАМЯТЬ: <предпочтение одним предложением от третьего лица>\n"
    "Если ничего такого нет, строку не добавляй."
)

# ИЗВЛЕЧЕНИЕ ПРЕДПОЧТЕНИЙ ДЕЛАЕТ МОДЕЛЬ, А НЕ ПАМЯТЬ, и вот почему.
#
# Предпочтения — худший тип вопросов на бенчмарке: R@1 17%. Разбор промахов
# показал, что дело не в весах и не в ранжировании. Улика выглядит так:
#
#     вопрос:  «посоветуй сериал на вечер»
#     улика:   «I'm looking for French podcasts that cover politics»
#
# Это не утверждение о вкусе, а ЗАПРОС, из которого вкус лишь выводится, и
# общих слов у них нет вовсе. Связывает их знание о мире.
#
# Что уже проверено и не помогло:
#   * надбавка к важности за речь о себе — ноль изменений; в разговоре с
#     ассистентом О СЕБЕ говорит КАЖДАЯ реплика (9 из 9 в сессии с уликой);
#   * метка и захват — R@1 65% -> 47%;
#   * вопрос как ворота к запоминанию — не подтвердилось на данных;
#   * модель вчетверо крупнее — разделяет ХУЖЕ (2/3 против 3/3).
#
# Модель же превращает запрос в утверждение одной строкой промпта. Память
# получает готовый факт и хранит его по своим правилам — разделение труда
# ровно такое, какое библиотека держит с самого начала.
MEMORY_MARK = "ПАМЯТЬ:"
...
def main() -> None:
    ...
    while True:
        ...
        # Модель могла выделить устойчивое предпочтение отдельной строкой.
        preference = ""
        if MEMORY_MARK in answer:
            answer, _, tail = answer.partition(MEMORY_MARK)
            answer = answer.rstrip()
            preference = tail.strip().splitlines()[0].strip() if tail.strip() else ""
