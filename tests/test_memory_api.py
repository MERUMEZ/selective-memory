...
def test_recall_resists_forgetting():
    ...
    ХРАНИЛИЩЕ НАРОЧНО НЕ ИЗ ДВУХ УЗЛОВ. При пороге поиска 0.10 в
    крошечной базе совпадает ВСЁ СО ВСЕМ: «нетронутый» узел тоже
    попадает в выдачу, тоже получает касание, и различать нечего.
    Первая версия проверки держала два узла и сломалась ровно так —
    обе стабильности стали 7.59.

    Заодно это настоящая находка о механизме: припоминание укрепляет
    ВСЁ, что вернуло, а не только использованное. Чем ниже порог, тем
    шире укрепление и тем слабее различение.
    """
    now = [1000.0]
    m = Memory(":memory:", clock=lambda: now[0])
    ...
    for filler in ("вчера чинили лифт в подъезде",
                   "картошка подорожала к осени",
                   "поезд опоздал на сорок минут",
                   "сосед завёл вторую кошку",
                   "в парке спилили старый тополь"):
        m.observe(filler, emotion=0.9)
