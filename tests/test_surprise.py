...
def _teach(graph: MemoryGraph, text: str, times: int) -> None:
    """
    Учит организм фразе ЦЕЛИКОМ: и словам, и самому событию.

    Прежде здесь было только process_language_input — то есть словарь.
    Этого хватало, пока удивление означало «незнакомость слов». Теперь оно
    означает РАССОГЛАСОВАНИЕ ПРЕДСКАЗАНИЯ, и знать слова мало: организм
    может выучить все слова фразы и всё равно не ждать её как события.

    Так и у людей: знакомые слова в невиданном сочетании — новость.
    Поэтому опыт здесь даётся полный, иначе тест меряет половину
    механизма и падает на второй.
    """
    for i in range(times):
        graph.process_language_input(text, timestamp=float(i))
        graph.save_connection(text, "", weight=0.5, timestamp=float(i))
