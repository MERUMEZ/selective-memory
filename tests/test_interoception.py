...
def test_interoception_is_on_by_default():
    """
    Умолчание не меняется молча — и здесь оно изменено ПОСЛЕ замера.

    LongMemEval, 500 вопросов: записано 43.9% -> 25.9% реплик при R@1
    96.8% -> 96.0%. Записей на 41% меньше ценой 0.8 пункта. Для памяти,
    вся суть которой в избирательности, размен выгодный.

    Проверка стоит здесь, чтобы обратное переключение тоже потребовало
    объяснения: значение трогает решение о записи у каждого пользователя
    библиотеки.
    """
