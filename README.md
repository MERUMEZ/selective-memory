...
| режим | записано узлов | R@1 | R@3 | R@5 | R@10 |
|---|---|---|---|---|---|
| рабочий | 1 602 | 13.3% | 16.7% | **18.3%** | 18.3% |
| архив (гейт обойдён, забывание выкл.) | 14 623 | 76.7% | 86.7% | **91.7%** | 96.7% |

Читать это надо так: **извлечение у нас хорошее, а спайк-гейт на этом
бенчмарке катастрофичен.** Как чистый поисковик мы даём 91.7% — тот же
порядок, что у соседей. Но гейт записывает лишь каждую девятую пару
реплик, а не записанное найти нечем.

Это не оправдание, а описание границы применимости. LongMemEval прячет
иголку в рутине — именно в том, что мы намеренно выбрасываем. Ассистенту
такой отбор нужен, поиску по стогу он противопоказан. Показательно, что
на вопросах об **обновлении знаний** архивный режим даёт 100%, а
рабочий 0%: механизм вытеснения версий работает, до него просто не
доходит.

Нужна полнота по стогу — отключайте гейт (`base_plasticity_threshold=0.0`)
или берите векторную базу.
