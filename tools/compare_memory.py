...
ВЕРСИИ ТЕЗИСА:
    ...
    скользящее окно  86.4%
    случайный отбор  90.0%
    организм         89.2%

Организм уверенно обыгрывает окно, но НЕ обыгрывает случайную выборку.
Одиночный прогон (сид 42) давал 94% против 90% и выглядел как победа —
на пяти сидах это шум.

Что подтвердилось: сжатие. Организм держит 10-13 узлов против 34-44 у
...
def main() -> None:
    ...
    for i, text in enumerate(stream, start=1):
        ...
        if args.session_length and args.gap_hours and i % args.session_length == 0:
            session.clock.simulate_elapsed_wall_seconds(args.gap_hours * 3600.0)
