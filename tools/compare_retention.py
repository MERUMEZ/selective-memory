...
РЕЗУЛЬТАТ на пяти сидах (1, 7, 13, 42, 99), молчание 14 суток:
    ...
    последние (окно)         28          77%      77%      +0%
    случайные (контроль)     29          43%      53%     -10%
    организм                 14         100%      70%     +30%

С КОНТРОЛЕМ НА ОБЪЁМ (--balanced, обе группы дают по два сообщения):
    последние (окно)         38          77%      77%      +0%
    случайные (контроль)     39          43%      53%     -10%
    организм                 16         100%      50%     +50%

Числа упали после поправки удивления на объём содержания (было +53/+67).
Это осознанная цена: без поправки библиотека записывала междометия
наравне с фактами — замер на потоке ассистента показал 6 записанных
"спасибо"/"окей"/"ага" из 6. Компромисс мерян по значениям параметра:
выключено даёт +67 и весь мусор, двойка — +50 и ноль мусора, тройка и
выше — +37 при том же нуле. Взята двойка.

Числа ВОСПРОИЗВОДИМЫ побайтово: часы стенда подменены (ManualWallClock),
эпоха мозга задана явно, выборка слогов ушла из ORDER BY RANDOM() в
seeded random. До этого стенд гулял на несколько пунктов между
запусками ОДНОГО И ТОГО ЖЕ кода, и точность вроде "+43" была
самообманом.
