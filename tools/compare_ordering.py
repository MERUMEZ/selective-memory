...
def main() -> None:
    ...
    if mrr_p - mrr_o > 0.05 and share_gap > 0.05:
        ...
        print(" Оговорка после перехода на умножение: раньше СРЕДИ ОДИНАКОВО")
        print(" ПОДХОДЯЩЕГО. Важность больше не обгоняет релевантность —")
        print(" похвала решает споры, а не отменяет соответствие вопросу.")
