...
**Дата:** 2026-08-01 · **Коммит:** `b8beace` · **Тестов:** 237 · **Строк Python:** 18 424
...
|---|---|---|
| Поиск линейный | **высокая для облака** | `fetch_searchable_nodes` поднимает ВСЕ узлы на каждый запрос. На ~400 узлах незаметно, на многопользовательском сервере это потолок |
| Семантика только русская | **высокая для продажи** | navec — русские векторы. Для геймдева и B2B нужен англоязычный путь |
